document.addEventListener('DOMContentLoaded', () => {
    ...
    const section2 = document.getElementById('activity-steps-2');
    ...
    // Section 2 Specifics
    const checkboxes2 = section2 ? section2.querySelectorAll('.step-checkbox') : [];
    const progressText2 = document.getElementById('progress-text-2');
    const progressBar2 = document.getElementById('progress-bar-2');
    const stepCards2 = section2 ? section2.querySelectorAll('.step-card') : [];

    // Combine for global tracking
    const allCheckboxes = [...checkboxes1, ...checkboxes2];
    const allStepCards = [...stepCards1, ...stepCards2];
    ...
    function updateProgress() {
        ...
        // --- Activity 2 Update ---
        const total2 = checkboxes2.length;
        const checked2 = Array.from(checkboxes2).filter(cb => cb.checked).length;
        if (progressText2) {
            progressText2.textContent = `${checked2}/${total2} Completed`;
        }
        if (progressBar2) {
            const percent2 = total2 > 0 ? (checked2 / total2) * 100 : 0;
            progressBar2.style.width = `${percent2}%`;
        }

        // --- Global Update ---
        const totalGlobal = total1 + total2;
        const checkedGlobal = checked1 + checked2;
        ...
        if(statusCircle && statusPercent) {
            ...
            if (percentGlobal === 100) {
                if(completionLabel) {
                    completionLabel.textContent = "Assignment Complete!";
                ...
                if(completionLabel) {
                    completionLabel.textContent = "Assignment Incomplete - Finish all 6 steps";
        ...
        // We map simply based on index in the combined array
        const state = allCheckboxes.map(cb => cb.checked);
        localStorage.setItem('automationWorkzWeek1State', JSON.stringify(state));
    ...
    // Load state
    const savedState = JSON.parse(localStorage.getItem('automationWorkzWeek1State'));
    if (savedState) {
        allCheckboxes.forEach((cb, index) => {
